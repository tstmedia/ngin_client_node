...
module.exports = function(ngin) {
  ...
  var Standings = SportsModel.extend({
    ...
    urlRoot: function(options) {
      ...
      else if (id = options.flight_stage_id || this.flight_stage_id) route.push('flight_stages', id)

      if (id = options.team_id || this.team_id) route.push('teams', id)
      else if (id = options.division_id || this.division_id) route.push('divisions', id)


      var base = config.urls && config.urls.sports || config.url
      console.log(route.join('/'))
      return Url.resolve(base, route.join('/') + '/standings')
    }
